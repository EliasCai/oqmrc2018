...
def create_data(data, ttv, 
    ...
    check_path = [passage_path, query_path, answer_path]
    ...
    return [passage_seq, query_seq, answer_seq], None
...
if __name__ == '__main__':  
    ...
    test_data = read_json(test_json_path,False)
    idx = test_data['alternatives'].map(lambda x: len(x.split('|'))) == 1
    test_data.loc[idx, 'alternatives'] = test_data.loc[idx, 'alternatives'].map(lambda x:x + '|无法确定' + '|无法确定')
    idx = test_data['alternatives'].map(lambda x: len(x.split('|'))) == 2
    test_data.loc[idx, 'alternatives'] = test_data.loc[idx, 'alternatives'].map(lambda x:x + '|无法确定' )
    ...
    test_inputs, test_y = create_data(test_data, 
                                ttv,passage_path='../log/test_passage.npy',
    ...
    # res = model.evaluate(np.hstack(test_inputs), eval_y)
    # print(res)
    ...
    test_pred = model.predict(np.hstack(test_inputs),batch_size=128)
    # find_best_prob(test_pred, eval_y, prob = 0.5)
    
    
    with codecs.open('../log/pred_0919_05.txt','w',encoding='utf-8') as f:
    
        for id, score, pred, alter in zip(test_data['query_id'].values,
                                   test_pred.max(axis=1),
                                   test_pred.argmax(axis=1),
                                   test_data['alternatives'].values):
            
            if score > 0.5:
                a = pred
            else:
                a = 0
            answer = alter.split('|')[a]
            print(str(id) + '\t' + answer)
